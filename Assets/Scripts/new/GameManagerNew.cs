...
public class GameManagerNew : MonoBehaviour
{
    ...
    public void CreateMiniGame(int level)
    {
        ...
        DOVirtual.DelayedCall(.5f, () =>
        {
            CurrentMiniGameStage = Instantiate(LevelManagerNew.Instance.miniStageList[level], new Vector2(0.1f, -3f), Quaternion.identity);
    ...
    IEnumerator NextStage()
    {
        ...
        {
            pictureUIManager.ChangeReaction(0.5f, "tremble", true, GameManagerNew.Instance.PictureUIManager.hasWindow);
        }
        else
        {
            pictureUIManager.ChangeReaction(0.5f, "idle_sad", true, GameManagerNew.Instance.PictureUIManager.hasWindow);
        }
    ...
    public void CheckStarValue(int numOfStar, Vector3 des, LevelButton levelButton)
    {
        if (SaveSystem.instance.star - numOfStar >= 0)
        {
            ...
            if (LevelManagerNew.Instance.LevelBase.Level == 3)
            {
                FirebaseAnalyticsControl.Instance.LogEventFixItemMap3(levelButton.level);
            }
            UIManagerNew.Instance.BlockPicCanvas.SetActive(true);
    ...
    public void PlayVideo()
    {
        ...
        Debug.LogError("chạy vào playvideo có videocontroller");
    }
    public void Vibration(HapticClip hapticClip)
    {
        hapticSouce.clip = hapticClip;
        if (hapticSouce.enabled)
        {
            hapticSouce.Play();
        }
    }
