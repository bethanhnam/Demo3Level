	...
	private void DisableIronPlate(Collider2D collision)
	{
		if (GameManagerNew.Instance.isMinigame)
		{
            if (collision.gameObject.tag == "Iron")
            {
                ...
                UIManagerNew.Instance.MiniGamePlay.miniItem[MiniGameStage.Instance.numOfIronPlates].gameObject.SetActive(true);
		...
		else
		{
			if (collision.gameObject.tag == "Iron")
			{
				...
				if (collision.GetComponent<IronPlate>().isEventItem)
				{
					...
					UIManagerNew.Instance.GamePlayPanel.EventCollectItemList.SpawnItem(collision.transform.position.x);
                    collision.GetComponent<IronPlate>().shiningParticle.Stop();
                }
                //AudioManager.instance.PlaySFX("DropIron");
