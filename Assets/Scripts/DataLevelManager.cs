...
public class DataLevelManager : MonoBehaviour
{
    ...
    private void RepairData()
    {
        ...
        {
            Debug.Log("chayj vaof tao repair data");
            if (LevelManagerNew.Instance.LevelBase.Level + 1 < DataLevelManager.Instance.DatatPictureScriptTableObjects.Length)
            {
                ...
                datalevel.Data = dataLevel.Data;
                for (int i = 0; i < LevelManagerNew.Instance.LevelBase.Level; i++)
                {
                    datalevel.Data[i] = dataLevel.Data[i];
                    datalevel.Data[i].IndexStage = dataLevel.Data[i].IndexStage;
                    datalevel.Data[i].Stage = dataLevel.Data[i].Stage;
                }
                dataLevel = new DataLevel();
                dataLevel.Data = new DataLevelChil[datatPictureScriptTableObjects.Length];
                for (int i = 0; i < LevelManagerNew.Instance.LevelBase.Level; i++)
                ...
                for (int i = LevelManagerNew.Instance.LevelBase.Level + 1; i < datatPictureScriptTableObjects.Length; i++)
                {
                    dataLevel.Data[i] = new DataLevelChil();
                    dataLevel.Data[i].IndexStage = 0;
                    dataLevel.Data[i].Stage = new DataStageLevel[datatPictureScriptTableObjects[i].Stage.Length];
                    ...
                    {
                        dataLevel.Data[i].Stage[j] = new DataStageLevel();
                        dataLevel.Data[LevelManagerNew.Instance.LevelBase.Level + i].Stage[j].DataItmeLevel = new DataItmeLevel[datatPictureScriptTableObjects[i].Stage[j].Item.Length];
                        for (int k = 0; k < dataLevel.Data[i].Stage[j].DataItmeLevel.Length; k++)
                        {
                            dataLevel.NumOfLevel++;
                            dataLevel.Data[i].Stage[j].DataItmeLevel[k] = new DataItmeLevel();
                            dataLevel.Data[i].Stage[j].DataItmeLevel[k].Id = datatPictureScriptTableObjects[LevelManagerNew.Instance.LevelBase.Level + i].Stage[j].Item[k].Id;
                            dataLevel.Data[i].Stage[j].DataItmeLevel[k].IsUnlock = false;
                            dataLevel.Data[i].Stage[j].DataItmeLevel[k].IndexSelect = 0;
                        }
                    }
