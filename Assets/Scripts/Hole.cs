...
public class Hole : MonoBehaviour
{
	...
	private void Update()
	{
		...
		if (!extraHole)
		{
			...
			{
				Collider2D[] colliders = Physics2D.OverlapCircleAll(this.transform.position, this.GetComponent<CircleCollider2D>().radius-0.13f , IronLayer);
	...
	private void OnDrawGizmos()
	{
		if (this.isActiveAndEnabled)
		{
			...
			Gizmos.DrawWireSphere(transform.position, this.GetComponent<CircleCollider2D>().radius- 0.13f);
		}
	}
